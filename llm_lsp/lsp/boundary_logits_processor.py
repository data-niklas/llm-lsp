    ...
    def __init__(self, tokenizer, token_boundaries, disabled=False):
        ...
        for boundary in token_boundaries:
            ...
            tokens_to_map = [
                ...
                if not token.endswith(boundary) and token.count(boundary) == 1
            ]
            ...
            def last_allowed_token(prefixes):
                return [
                    ...
                    if prefix.endswith(boundary) or boundary not in prefix
                ][-1]

            token_map = {
                token: last_allowed_token(trie.prefixes(token))
